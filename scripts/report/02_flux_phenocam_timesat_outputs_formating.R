...
### fixed thresholds
files <- Sys.glob("data/3_timesat_outputs/phenometrics/flux_fix/*.csv") %>%
  str_subset("(GPP_loess|PPI_M_loeFiltered|EVI2_M_loeFiltered|NDVI_M_loeFiltered|PPI_raw_loeFiltered|EVI2_raw_loeFiltered|NDVI_raw_loeFiltered)")

fluxVIphenofix <- NULL
for (i in files) {
  tmp <- str_extract(i, "\\d.\\d{2}_(\\w{3,4})_([:alpha:]{1,3})")
  thres <- str_split(tmp, "_", simplify = TRUE)[1] 
  VIname <- str_split(tmp, "_", simplify = TRUE)[2]
  BRDF <- str_split(tmp, "_", simplify = TRUE)[3]
  tmp <- read_csv(i) %>% 
  ...
  fluxVIphenofix <- bind_rows(fluxVIphenofix, tmp)
}
...
### fixed thresholds
files <- Sys.glob("data/3_timesat_outputs/phenometrics/phenocam_fix/*.csv") %>%
  str_subset("(GCC_loess|PPI_M_loeFiltered|EVI2_M_loeFiltered|NDVI_M_loeFiltered|PPI_raw_loeFiltered|EVI2_raw_loeFiltered|NDVI_raw_loeFiltered)")

phenocamVIphenofix <- NULL
for (i in files) {
  tmp <- str_extract(i, "\\d.\\d{2}_(\\w{3,4})_(\\w{1,3})")
  thres <- str_split(tmp, "_", simplify = TRUE)[1]
  VIname <- str_split(tmp, "_", simplify = TRUE)[2]
  BRDF <- str_split(tmp, "_", simplify = TRUE)[3]
  tmp <- read_csv(i) %>% 
  ...
  phenocamVIphenofix <- bind_rows(phenocamVIphenofix, tmp)
}
...
phenocamVIphenofix <- phenocamVIphenofix %>% 
  mutate(LC = recode(LC, !!!level_key))
...
## TIMESAT phenometrics amplitude threshold
...
## TIMESAT phenometrics fixed threshold
fluxVIphenofix1 <- fluxVIphenofix %>% 
  mutate(validate = "GPP")
phenocamVIphenofix1 <- phenocamVIphenofix %>% 
  mutate(validate = "GCC") %>% 
  # completely mismatch GCC and VI, likely due to large uncertaities in GCC location
  filter(!(ID %in% c("monteblanco_SH_1000", "monteblanco_EB_1000", "montenegro_SH_1000",
                     "donanafuenteduque_WL_1000", "DE-Geb_AG_1000", "borgocioffinorth_AG_1000")))

VIphenofix <- bind_rows(fluxVIphenofix1, phenocamVIphenofix1) 
save(VIphenofix, file = "data/RData/VIpheno_flux_phenocam_fix.RData")