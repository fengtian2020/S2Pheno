...
phenoThres("M")
ggsave("figures/report/figure_phenometrics_compare_to_GPP_GCC_NBAR_amplitude.pdf",
       width = 25, height = 25, units = "cm")

phenoThres("raw")
ggsave("figures/report/figure_phenometrics_compare_to_GPP_GCC_raw_amplitude.pdf",
       width = 25, height = 25, units = "cm")
...
load("data/RData/VIpheno_flux_phenocam_fix.RData")
...
phenoData <- function(brdfm, vali){
  ...
  Gpheno <-
    ...
    mutate(VIname = factor(VIname, levels = c("NDVI", "EVI2", "PPI")),
           metric = factor(metric, levels = c("SOS", "EOS"))) %>% 
    mutate(diff = abs(Gvar-VI),
           thres_VI = as.numeric(thres_VI) * 10,
           thres_Gvar = as.character(as.numeric(thres_Gvar) * 10),
           validate = vali)
  return(Gpheno)
}
phenoThres <- function(brdfm) {
  bind_rows(phenoData(brdfm, "GPP"), phenoData(brdfm, "GCC")) %>% 
    ...
    ggplot(aes(x = thres_VI, color = VIname)) +
    geom_vline(xintercept = 5, color = "grey60", linetype = 2)+
    geom_point(aes(y = val, shape = measure), size = 2.5) +
    geom_line(aes(y = val, linetype = measure), size = .8) +
    geom_text(aes(y = val, label = thres_Gvar),
              position = position_nudge(y = -2))+
    ...
    facet_grid(rows = vars(metric), cols = vars(validate), scales = "free")+
    theme_bw(base_size = 17)+
    labs(x = "VI fixed threshold level") +
    theme(panel.grid = element_blank(),
          legend.position = "top",
          legend.title = element_blank(),
          legend.text = element_text(size = 17))
...
phenoThres("M")
ggsave("figures/report/figure_phenometrics_compare_to_GPP_GCC_NBAR_fixed.pdf",
       width = 25, height = 25, units = "cm")

phenoThres("raw")
ggsave("figures/report/figure_phenometrics_compare_to_GPP_GCC_raw_fixed.pdf",
       width = 25, height = 25, units = "cm")
