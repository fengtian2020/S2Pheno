...
pepS2 <- raw %>% 
  ...
  # for report, 70% sites
  filter(!(s_id %in% c(read_csv("data/0_ground_data_raw/pep725/PEP_site_for_valiation.csv") %>% pull(s_id))))

save(pepS2, file = "data/RData/pepS2report.RData")
...
#   filter(!(s_id %in% c(read_csv("data/0_ground_data_raw/pep725/PEP_site_for_valiation.csv") %>% pull(s_id))))
# 
# save(pepS2, file = "data/RData/pepS2reportfix.RData")